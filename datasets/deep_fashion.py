...
# path: /home/guy/sd1tb/datasets/deep_fashion/DeepFashion/Category and Attribute Prediction Benchmark/Img/preprocessed_img
# dataset statistices : 
#   mean: [0.7678, 0.7364, 0.7283]
#   std: [0.3057, 0.3189, 0.3203]

class DeepFashionCATP(Dataset):
    '''
    DeepFashion - Category and Attributes Prediction
    URL: 
    '''
    def __init__(self, dataset_path, split='train',transform=None,**kwargs):
        self.files = [i for i in Glob(f'{dataset_path}/Img/preprocessed_img/**/*.jpg',recursive=True)]      # Category-and-Attribute-Prediction
        # read item attributes:
        # the attribute can be the cloth attribute or it can be viewing angle or type of item (based on the folder)
        # in the following example, we're taking the cloth attributes (463 attributes)
        anno_filename = os.path.join(dataset_path,'Anno_coarse','list_attr_img.txt')
        with open(anno_filename,'r') as f:
            attr_items=f.read().splitlines()
        attr_items=attr_items[2:]
        attr_items=[i.split() for i in attr_items]
        self.attr_items = {i[0].split('img/')[1]:[int(int(a)>0) for a in i[1:]] for i in attr_items}
        # get attributes names
        anno_filename = os.path.join(dataset_path,'Anno_coarse','list_attr_cloth.txt')
        with open(anno_filename,'r') as f:
            attr_cloth=f.read().splitlines()
        self.attr_names = [a.rstrip('  0123456789') for a in attr_cloth[2:]]
    ...
    def __getitem__(self, ix):
        fpath = self.files[ix]
        lbl=self.attr_items[fpath.split('img/')[1:][0]]
        img = read_image(fpath)/255.0
        return img,lbl
